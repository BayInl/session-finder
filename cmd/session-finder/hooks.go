...
const (
	...
	// Keep this command intentionally boring. It checks the binary, takes a
	// process-local lock, and starts a detached scan before returning to the host.
	hookCommand = `sh -c 'command -v session-finder >/dev/null 2>&1 || exit 0; lock_dir="${TMPDIR:-/tmp}/session-finder-extract.lock"; mkdir "$lock_dir" 2>/dev/null || exit 0; (nohup session-finder skill extract --pending >/dev/null 2>&1 </dev/null; rmdir "$lock_dir" 2>/dev/null || true) &'`
